...
function carregaDadosJson() {
	$.ajax({
		...
		.done(function (result) {
			...
			table +=
				'<tr><th>Id</th><th>Fabricante</th><th>Modelos</th><th>Ano</th><th>Cor</th><th>Valor</th></tr>';
...
function filtrarDados(target, value) {
	if (target == 'Fabricante') {
		if (value != 'Outros') {
			...
			$(
				"#veiculos td.col_fabricante:not(:contains('Toyota')), #veiculos td.col_fabricante:not(:contains('Ford')), #veiculos td.col_fabricante:not(:contains('BMW')), #veiculos td.col_fabricante:not(:contains('Dodge'))",
			)
				.parent()
				.show();
			$(
				"#veiculos td.col_fabricante:contains('Toyota'), #veiculos td.col_fabricante:contains('Ford'), #veiculos td.col_fabricante:contains('BMW'), #veiculos td.col_fabricante:contains('Dodge')",
			)
				.parent()
				.hide();
		}
